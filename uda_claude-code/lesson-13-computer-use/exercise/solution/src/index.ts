...
1. TOOL CONFIGURATION
   - Use "computer_20250124" tool type
   - Specify display dimensions
   - Include in betas: ["computer-use-2025-01-24"]

2. AGENT LOOP PATTERN
   - Send task with screenshot request
   - Process tool_use blocks for actions
   - Return screenshots as image content
   - Continue until end_turn or limit reached

3. ACTION TYPES
   - screenshot: Capture current state
   - left_click: Click at coordinates
   - type: Enter text
   - key: Press keys (Tab, Enter, etc.)
   - wait: Pause for animations

4. SAFETY MEASURES
   - Limit max actions per session
